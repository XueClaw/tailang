...
- 文本相等 / 不等判断
- 英文 `&&` / `||` / `!` 逻辑运算
- self-native 与 LLVM 路径的标量元素运行时数组字面量与下标访问
- self-native 与 LLVM 路径的标量成员运行时对象字面量与成员/字符串键读取

当前仍未完成或未打通的能力包括：

- 更深层的 self-native 运行时对象一致性
- 更完整的文本运行时
- 多平台原生产物
...
- `llvm_backend.rs`
  - 承担正式对象运行时路径
  - 允许标量与深层容器数组字面量、数组下标访问、数组值返回
  - 允许标量成员与容器成员对象字面量、成员读取、字符串键下标读取
- `codegen.rs`
  - self-native 已支持标量元素运行时数组字面量和下标读取
  - self-native 已支持标量成员运行时对象字面量、成员读取、字符串键下标读取
  - 对更深层对象/容器成员形状会显式要求改用 `--backend llvm`
- `hir.rs`
  - 常量对象仍可走常量折叠链
  - 显式 `object` 局部变量现在可进入运行时对象语义
