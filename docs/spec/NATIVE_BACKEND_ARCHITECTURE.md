...
- 文本相等 / 不等判断
- 英文 `&&` / `||` / `!` 逻辑运算
- self-native 与 LLVM 路径的运行时数组字面量与下标访问
- self-native 与 LLVM 路径的运行时对象字面量与成员/字符串键读取
...
- `llvm_backend.rs`
  - 承担正式对象运行时路径
  - 允许数组字面量、数组下标访问、数组值返回
  - 允许标量成员对象字面量、成员读取、字符串键下标读取
- `codegen.rs`
  - self-native 已支持运行时数组字面量和下标读取
  - self-native 已支持运行时对象字面量、成员读取、字符串键下标读取
  - 更深层对象能力仍在继续向 LLVM 路径收敛
- `hir.rs`
  - 常量对象仍可走常量折叠链
